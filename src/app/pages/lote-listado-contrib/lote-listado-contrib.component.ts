...
@Component({
  selector: 'app-lote-listado-contrib',
  templateUrl: './lote-listado-contrib.component.html',
  styleUrls: ['./lote-listado-contrib.component.scss']
})
export class LoteListadoContribComponent implements OnInit {
  ...
  loadDataProceso() {
    ...
    this.api.getDataLoteDetalleListar(data_post).subscribe((data: any) => {
      console.log(data);

      this.litNumProceso = this.paramNumProceso.padStart(4, '0');
      this.litTituloA = data[0].elc_destri.toUpperCase();
    });
  }
  ...
  detalleProccon() {
    this.router.navigate(['/lote-deuda-contrib', this.paramNumProceso, this.actualDetalleProcconId]);
  }
